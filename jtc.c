 ...
 * that only supports vt[123]xx/ANSI X3.64/ECMA-48/ISO6429.
 * Since Jove uses a very small set of terminal capabilites,
 * and since everyone in the year 2020CE uses a terminal emulator
 * anyway, that supports far more than these, the approach here
 * is to define a single nested set of capabilities, use
 * from JOVEVT or TERM to select a level and fake out
 * the terminfo routines.  If someone finds themselves
 * on a terminal that does not support the necessary minimal
 * subset of ANSI X.3, guess you need to compile jove with
 * one of termcap/terminfo/curses/ncurses/ncursesw!
 */
...
extern int usleep proto((unsigned));
...
#define SitFor(n) usleep(n*100000)
...
#define VT320 320 /* SU/SD to pan up or down by multiple lines */
...
private JTermcap jtc[] = {
	...
	{"ks", "\033[?1h\033=", VT100},
	{"ke", "\033[?1l\033>", VT100},
	{"ti", "\0337\033[?47h", VTALT}, /* \033[?1049h */
	{"te", "\033[2J\033[?47l\0338", VTALT}, /* \033[?1049l */
	{"bl", "\007", 0},
	{"AL", "\033[%dL", VT102},
	{"DL", "\033[%dM", VT102},
...
private JTLevel jtc_emu[] = {
    ...
    {"kterm", VTALT},
    {"vte", VTALT},
    {"st", VT125}, /* does not support SR, even though it does have ALT */
};
...
const char *tenv;
{
	...
	for (j = jtc_emu; j < jtc_emu + (sizeof(jtc_emu)/sizeof(jtc_emu[0])); j++) {
...
const char *capname;
{
     ...
     * which are the only ones with xterm etc where NP might
     * be true.
     */
    return NO;
}
...
char **area;
{
	...
	if (capeq(j->capname, capname) && j->caplevel <= jtlev) {
	    if (area && *area) {
		size_t caplen = strlen(j->capval) + 1; /* include NUL */
